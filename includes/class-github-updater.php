 ...
 *
 * @package Entry_Normalizer_For_Gravity_Forms
 */
...
class GFEN_GitHub_Updater
{
     ...
     * @var string
     */
    private const GITHUB_REPO = 'entry-normalizer-for-gravity-forms';
     ...
     * @var string
     */
    private const PLUGIN_FILE = 'entry-normalizer-for-gravity-forms/entry-normalizer-for-gravity-forms.php';
     ...
     * @var string
     */
    private const PLUGIN_SLUG = 'entry-normalizer-for-gravity-forms';
     ...
     * @var string
     */
    private const PLUGIN_NAME = 'Entry Normalizer for Gravity Forms';
     ...
     * @var string
     */
    private const PLUGIN_DESCRIPTION = 'Normalize Gravity Forms fields (existing entries and future submissions) from BEFORE/AFTER examples: the plugin automatically detects the matching transformation.';
     ...
     * @var string
     */
    private const TEXT_DOMAIN = 'entry-normalizer-for-gravity-forms';
     ...
     * @var string
     */
    private const CACHE_KEY = 'gfen_github_release';
    ...
    {
        if (defined('GFEN_BASENAME') && is_string(GFEN_BASENAME) && '' !== GFEN_BASENAME) {
            return GFEN_BASENAME;
        }
    ...
    private static function build_plugin_info_result(): stdClass
    {
        ...
        // Resolve installed version (prefer the plugin's constant, fallback safely to get_plugin_data)
        $installed_version = '1.0.0';
        if (defined('GFEN_VERSION')) {
            $installed_version = GFEN_VERSION;
        } else {
    ...
    private static function build_fallback_plugin_info_result(): stdClass
    {
        ...
        $result->version      = defined('GFEN_VERSION') ? GFEN_VERSION : '1.0.0';
    ...
    public static function plugin_info_css(): void
    {
        ...
        // and inject the Gravity Forms sidebar line.
        echo '<script>'
        ...
        // --- Gravity Forms add-on: add "Requires Gravity Forms" sidebar line ---
        // (This plugin always requires GF, so the line is always injected.)
    ...
    private static function parse_readme(): array
    {
        ...
        for ($i = 1, $count = count($parts); $i < $count; $i++) {
            ...
            if ('installation' === $title) {
                ...
                // Include in description (e.g. "Example-Based Detection", "Key Features").
                $description .= "\n\n## " . trim($lines[0]) . "\n" . $body;
            }
        }
